...
export default function useLogin() {
    ...
    React.useEffect(() => {
        if (Boolean(user)) {
            const redirectionPath = user.type === 'admin' ? '/admin/dashboard' : '/user';
    ...
    const submitLogin = async event => {
        ...
        const response = await postRequest('http://localhost:3000/api/auth/login', { email, password });
        console.log(response);

        if (Boolean(response.token)) {
            localStorage.setItem('next_auth_token', response.token);
            setUser(response);
            const redirectionPath = response.type === 'admin' ? '/admin/dashboard' : '/user';
            router.push(redirectionPath);
        }
    };
